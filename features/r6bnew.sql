...
WHERE
    ...
    AND a.originalTitle LIKE '%love%'
    -- Release year range filter.
    AND a.releaseYear BETWEEN 1990 AND 2020
    -- Average rating range filter.
    AND a.averageRating BETWEEN 7.0 AND 9.0
    -- Genre filter: ensure title exists in relatedGenres with a genre containing "Comedy".
    AND EXISTS (
        SELECT 1
        FROM relatedGenres rg
        WHERE rg.tid = a.tID
        AND rg.genre LIKE '%Comedy%'
    )
